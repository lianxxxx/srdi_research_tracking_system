...
$pendingPapers = $db->getPendingResearchPapers();
?>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card shadow-sm">
                    <div class="card-body">
                        <h5 class="card-title text-center mb-4">Pending Research Papers</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover" id="datatable">
                                ...
                                <tbody>
                                        ...
                                        <?php foreach ($pendingPapers as $paper): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <span
                                                        class="badge 
                                                        <?= $paper['research_status'] === 'Approved' ? 'bg-success' :
                                                            ($paper['research_status'] === 'Declined' ? 'bg-danger' :
                                                                ($paper['research_status'] === 'Revised' ? 'bg-warning text-dark' : 'bg-secondary')); ?>">
                                                        <?= htmlspecialchars($paper['research_status']); ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <form method="POST" style="display:inline-block;">
                                                        <input type="hidden" name="id" value="<?= $paper['id']; ?>">
                                                        <input type="hidden" name="research_status" value="Approved">
                                                        <button type="submit" name="update_status"
                                                            class="btn btn-sm btn-success"
                                                            onclick="return confirm('Approve this research?');">
                                                            Approve
                                                        </button>
                                                    </form>
                                                    <form method="POST" style="display:inline-block;">
                                                        <input type="hidden" name="id" value="<?= $paper['id']; ?>">
                                                        <input type="hidden" name="research_status" value="Declined">
                                                        <button type="submit" name="update_status" class="btn btn-sm btn-danger"
                                                            onclick="return confirm('Decline this research?');">
                                                            Decline
                                                        </button>
                                                    </form>
                                                    <form method="POST" style="display:inline-block;">
                                                        <input type="hidden" name="id" value="<?= $paper['id']; ?>">
                                                        <input type="hidden" name="research_status" value="Revised">
                                                        <button type="submit" name="update_status"
                                                            class="btn btn-sm btn-warning"
                                                            onclick="return confirm('Mark this research as revised?');">
                                                            Revise
                                                        </button>
                                                    </form>
                                                </td>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="9" class="text-center text-muted">No pending research papers found.
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
